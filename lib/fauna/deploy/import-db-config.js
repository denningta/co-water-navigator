...
const importDbConfig = async (path) => {
  ...
  try {
    const collectionsRes = await faunaClient.query(
      q.Let({ config: config },
        importCollections(q.Var('config'))
      )
    )
    console.log(`${collectionsRes.length} collections imported successfully`)

    const indexesRes = await faunaClient.query(
      q.Let({ config: config },
        importIndexes(q.Var('config'))  
      )
    )
    console.log(`${indexesRes.length} indexes imported successfully`)

    const functionsRes = await faunaClient.query(
      q.Let({ config: config },
        importFunctions(q.Var('config'))  
      )
    )
    console.log(`${functionsRes.length} functions imported successfully`)

    const rolesRes = await faunaClient.query(
      q.Let({ config: config },
        importRoles(q.Var('config'))  
      )
    )
    console.log(`${rolesRes.length} roles imported successfully`)
