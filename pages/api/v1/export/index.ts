...
import { PageSizes, PDFDocument, PDFFont, PDFPageDrawRectangleOptions, rgb, StandardFonts, TextAlignment } from "pdf-lib";
import { title } from "process";
import { drawTable, getTextObj, PDFColDef, TextOptions } from "../../../../lib/pdf-lib/helpers";
...
const createPdf = async () => {
  ...
  addDbb004(pdfDoc, [helvetica, helveticaBold], false)

  const pdfBytes = await pdfDoc.save()
  return pdfBytes
}

const addDbb004 = async (pdfDoc: PDFDocument, fonts: PDFFont[], debug: boolean = true) => {
  ...
  const wellDetailsText = [
    getTextObj(
      ...
      { ...defaultTextOptions, font: helveticaBold },
      0,
      wellDetailsBox,
      'left'
    ),
